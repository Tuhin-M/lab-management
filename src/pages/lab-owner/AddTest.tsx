...
import { Info, Trash2, ArrowLeft, FlaskConical, Plus } from "lucide-react";
...
const testSchema = z.object({
  ...
  sampleType: z.enum(["Blood", "Urine", "Saliva", "Stool", "Swab", "Tissue", "Other"], {
    required_error: "Please select a sample type",
  }),
  price: z
    .number({ required_error: "Price is required", invalid_type_error: "Price must be a number" })
    .positive({ message: "Price must be greater than ₹0" })
    .max(100000, { message: "Price seems too high — maximum is ₹1,00,000" }),
  discount: z
    ...
    .array(
      z.object({
        name: z
          ...
          .max(100, { message: "Parameter name too long" }),
      })
    )
    .min(1, { message: "Add at least one test parameter" })
    .max(30, { message: "Maximum 30 parameters allowed" }),
});
...
const AddTest = () => {
  ...
  const onSubmit = async (data: TestFormValues) => {
    if (!labId) {
      toast.error("Lab ID is missing. Go back and try again from the lab dashboard.");
      return;
    }
    try {
      await labOwnerAPI.addTestToLab(labId, data);
      toast.success("Test added successfully to your lab's catalog!");
      navigate(`/lab-owner/lab/${labId}`);
    } catch (error) {
      console.error("Failed to add test:", error);
      toast.error("Failed to add test. Please check your details and try again.");
    }
  };
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                ...
                <div className="space-y-6">
                  <h3 className="text-base font-bold text-slate-700 uppercase tracking-wide border-b pb-2">Test Information</h3>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="Describe what this test measures, who it's for, and any fasting or preparation requirements..."
                ...
                {/* --- Pricing & Timing --- */}
                <div className="space-y-6">
                  <h3 className="text-base font-bold text-slate-700 uppercase tracking-wide border-b pb-2">Pricing & Timing</h3>
                            ...
                            <TooltipProvider>
                              <Tooltip>
                                ...
                                <TooltipContent className="rounded-lg max-w-xs">
                                  Time until report is ready. e.g. "24 hours", "2 days", "6 hrs"
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          </FormLabel>
                          ...
                          <FormLabel className="text-slate-700 font-semibold flex items-center gap-1">
                            ...
                            <TooltipProvider>
                              <Tooltip>
                                ...
                                <TooltipContent className="rounded-lg max-w-xs">
                                  How long the collection appointment takes. e.g. "15 mins", "1 hour"
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          </FormLabel>
                ...
                <div className="bg-slate-50/60 p-6 rounded-3xl border border-slate-100 space-y-4">
                  ...
                  <p className="text-xs text-slate-500">
                    Add the specific biomarkers or values this test measures (e.g. Hemoglobin, WBC Count, Cholesterol).
                  </p>
                ...
                <Button
                  ...
                  className="w-full h-14 text-lg font-extrabold rounded-2xl shadow-xl shadow-primary/20 hover:scale-[1.01] transition-all bg-gradient-to-r from-primary to-blue-600 hover:from-primary/90 hover:to-blue-700 mt-4"
                >
                  {form.formState.isSubmitting ? "Saving..." : "Save & Add Test to Catalog"}
                </Button>
              </form>
            </Form>
          </CardContent>
